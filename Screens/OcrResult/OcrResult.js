...
if(selectenum!=null && commerce.length>3){
  setload('block')
    ...
    if ((key+1) == selectenum) {
        ...
        return { ...item, depense: [...item.depense,{id:Math.floor(Math.random()*1000000000),montant:convertedNumber,date:dateString,time:formattedTime,type:'scan',commerce:commerce}] };
      }
...
else if(selectenum==null){
  setload('none')
  alert("Categorie ?")
}

else if(commerce.length<2){
  setload('none')
  alert("nom du commerce ?")
}
...
<View style={{display:"flex",justifyContent:"space-around",width:"100%",height:"58%",paddingHorizontal:"5%",paddingVertical:"10%"}}>
      ...
      <Text style={{fontSize:getResponsiveFontSize(21),fontFamily:"PoppinsMedium",color:'black'}}>{prix.toFixed(2)}Dh  Validee ? </Text>        
       ...
       <ModalSelector
               ...
               <Text style={{fontSize:getResponsiveFontSize(16),fontFamily:"PoppinsRegular",color:'black',marginTop:'5%'}}>Nom du commerce :</Text>
               <TextInput 
                  onChangeText={setCommerce}
                  value={commerce}
                  style={{width:'100%',padding:"0%", borderColor:'#F0F0F0',paddingVertical:'2.4%',paddingLeft:'5%',borderRadius:5,marginTop:'4%',backgroundColor:'#F3F3FC',marginLeft:'0%',fontSize:getResponsiveFontSize(14),fontFamily:"PoppinsRegular",color:'#484948',    }}
                  placeholder=""
